...
function drawCards() {
    if (cardDeckStatus == false) {
        ...
        for (let index = 0; index < 5; index++) {
            const randomCard = Math.floor(Math.random() * cardDeck.length + [0]);
            handCard[index] = cardDeck[randomCard];
            cardDeck[randomCard] = "z";
            cardDeck.sort();
            cardDeck.pop(5);
            document.getElementById("cards" + index).innerHTML = ("<img src='cards/" + (handCard[index]) + ".png");
            id = "" + index;
            var onclick, id, ev;
            any;
